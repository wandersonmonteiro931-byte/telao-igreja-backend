...
export interface IStorage {
  ...
  getSecurityQuestions(identifier: string): Promise<{
    question1: string;
    question2: string;
    question3: string;
  } | null>;
  ...
  updateUserSettings(
    userId: number,
    data: Partial<{ churchName: string }>
...
// ------------------------------
// FUNÇÃO PARA CRIAR ADMIN INICIAL
// ------------------------------
...
function getInitialUsers(): {
  ...
  if (adminEmail && adminPassword) {
    ...
    console.log(`Admin inicial configurado: ${adminEmail}`);
  }

  return users;
}
// ============================
//   STORAGE.ts — Parte 2/5
//   Classe: MemStorage
// ============================
...
export class MemStorage implements IStorage {
  ...
  async init() {
    ...
    for (const u of initialUsers) {
      ...
      if (!existing) {
        await this.addUser(u.email, u.username, "", u.password, u.role, {
  ...
  async verifySecurityAnswers(userId: number, answer1: string, answer2: string, answer3: string): Promise<boolean> {
    ...
    const m1 = await bcrypt.compare(answer1.toLowerCase().trim(), user.securityAnswer1);
    const m2 = await bcrypt.compare(answer2.toLowerCase().trim(), user.securityAnswer2);
    const m3 = await bcrypt.compare(answer3.toLowerCase().trim(), user.securityAnswer3);
    return m1 && m2 && m3;
  }
  ...
  async updatePassword(userId: number, newPassword: string): Promise<boolean> {
    ...
    const updated = await this.updateUser(userId, { passwordHash });
    return updated !== null;
  }
  ...
  async resetFailedAttempts(id: number): Promise<void> {
    await this.updateUser(id, { failedAttempts: 0, lastFailedAttempt: null });
  }
  ...
  async getLoginAttempts(userId?: number, limit: number = 50): Promise<LoginAttempt[]> {
    let list = [...this.loginAttempts];
    if (userId !== undefined) list = list.filter(l => l.userId === userId);
  ...
  async getRecentFailedAttempts(email: string, minutes: number): Promise<number> {
    ...
    return this.loginAttempts.filter(a => a.email === email && !a.success && a.createdAt >= cutoff).length;
  }

  async getRecentFailedAttemptsByIp(ipAddress: string, minutes: number): Promise<number> {
    const cutoff = new Date(Date.now() - minutes * 60 * 1000);
    return this.loginAttempts.filter(a => a.ipAddress === ipAddress && !a.success && a.createdAt >= cutoff).length;
  }
  ...
  async updateAnnouncement(id: number, data: Partial<InsertAnnouncement>): Promise<Announcement | null> {
    ...
    if (idx === -1) return null;
    this.announcements[idx] = { ...this.announcements[idx], ...data, updatedAt: new Date() };
  ...
  async updateUserSettings(userId: number, data: Partial<{ churchName: string }>): Promise<UserSettings> {
    ...
    if (existing) {
      ...
      this.userSettings[idx] = { ...existing, ...data, updatedAt: new Date() };
...
// ============================
//   STORAGE.ts — Parte 3/5
//   Classe: FileStorage + export
// ============================
...
// escolha de armazenamento — ajuste se quiser usar MemStorage
