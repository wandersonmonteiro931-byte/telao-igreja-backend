import { type User, type InsertUser, type LoginAttempt, type InsertLoginAttempt, type Announcement, type InsertAnnouncement, type UserSettings, type SupportTicket, type InsertSupportTicket, type UpdateSupportTicket, type SupportTicketStatus } from "@shared/schema";
...
export interface IStorage {
  ...
  incrementSecurityAnswerAttempts(id: number): Promise<number>;
  resetSecurityAnswerAttempts(id: number): Promise<void>;
  blockRecovery(id: number): Promise<void>;
  unblockRecovery(id: number): Promise<void>;
  isRecoveryBlocked(id: number): Promise<boolean>;
...
export class MemStorage implements IStorage {
  ...
  async getUserByEmail(email: string): Promise<User | null> {
    ...
    if (user) {
      return {
        ...user,
        securityAnswerAttempts: user.securityAnswerAttempts ?? 0,
        isRecoveryBlocked: user.isRecoveryBlocked ?? false,
      };
    }
    return null;
  }
  ...
  async getUserByPhone(phone: string): Promise<User | null> {
    const user = this.users.find(u => u.phone === phone) || null;
    if (user) {
      return {
        ...user,
        securityAnswerAttempts: user.securityAnswerAttempts ?? 0,
  ...
  async getUserByIdentifier(identifier: string): Promise<User | null> {
    const user = this.users.find(u => u.email === identifier || u.username === identifier) || null;
    if (user) {
      return {
        ...user,
        securityAnswerAttempts: user.securityAnswerAttempts ?? 0,
  ...
  async updateUser(id: number, updates: Partial<User>): Promise<User | null> {
    ...
    return {
      ...this.users[userIndex],
      securityAnswerAttempts: this.users[userIndex].securityAnswerAttempts ?? 0,
      isRecoveryBlocked: this.users[userIndex].isRecoveryBlocked ?? false,
    };
  }
  ...
  async incrementSecurityAnswerAttempts(id: number): Promise<number> {
    const user = await this.getUserById(id);
    if (user) {
      const newAttempts = (user.securityAnswerAttempts || 0) + 1;
      await this.updateUser(id, { securityAnswerAttempts: newAttempts });
      return newAttempts;
    }
    return 0;
  }

  async resetSecurityAnswerAttempts(id: number): Promise<void> {
    await this.updateUser(id, { securityAnswerAttempts: 0 });
  }

  async blockRecovery(id: number): Promise<void> {
    await this.updateUser(id, { isRecoveryBlocked: true });
  }

  async unblockRecovery(id: number): Promise<void> {
    await this.updateUser(id, { isRecoveryBlocked: false, securityAnswerAttempts: 0 });
  }

  async isRecoveryBlocked(id: number): Promise<boolean> {
    const user = await this.getUserById(id);
    return user?.isRecoveryBlocked || false;
  }
...
export class DatabaseStorage implements IStorage {
  ...
  async incrementSecurityAnswerAttempts(id: number): Promise<number> {
    ...
    if (user) {
      ...
      await db
        .update(users)
        .set({ securityAnswerAttempts: newAttempts })
        .where(eq(users.id, id));
      return newAttempts;
    }
    return 0;
  ...
  async blockRecovery(id: number): Promise<void> {
    ...
    const { users } = await import("@shared/schema");
    const { eq } = await import("drizzle-orm");
    await db.update(users).set({ isRecoveryBlocked: true }).where(eq(users.id, id));
  }
  ...
  async unblockRecovery(id: number): Promise<void> {
    ...
    await db
      .update(users)
      .set({ isRecoveryBlocked: false, securityAnswerAttempts: 0 })
      .where(eq(users.id, id));
  }
  ...
  private announcementsCache: Announcement[] = [];
  ...
  async createAnnouncement(data: InsertAnnouncement): Promise<Announcement> {
    ...
    this.announcementsCache.push(announcement);
    return announcement;
  }

  async getAnnouncements(): Promise<Announcement[]> {
    return [...this.announcementsCache].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
  }
  ...
  async getAnnouncementById(id: number): Promise<Announcement | null> {
    return this.announcementsCache.find(a => a.id === id) || null;
  }

  async updateAnnouncement(id: number, data: Partial<InsertAnnouncement>): Promise<Announcement | null> {
    const idx = this.announcementsCache.findIndex(a => a.id === id);
    if (idx === -1) return null;
    this.announcementsCache[idx] = {
      ...this.announcementsCache[idx],
      ...data,
      updatedAt: new Date(),
    };
    return this.announcementsCache[idx];
  }

  async deleteAnnouncement(id: number): Promise<boolean> {
    const idx = this.announcementsCache.findIndex(a => a.id === id);
    if (idx === -1) return false;
    this.announcementsCache.splice(idx, 1);
    return true;
  }
