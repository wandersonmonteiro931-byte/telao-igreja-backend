...
export class FirebaseStorage implements IStorage {
  ...
  private async getNextId(counterName: string): Promise<number> {
    ...
    const { FieldValue } = await import("./firebase");
  ...
  async addUser(
    ...
    await db.collection("users").doc(String(nextId)).set({
      ...user,
  ...
  private docToUser(doc: FirebaseFirestore.DocumentSnapshot): User | null {
  ...
  async updateUser(id: number, updates: Partial<User>): Promise<User | null> {
    ...
    if ('lastFailedAttempt' in updates) {
      updateData.lastFailedAttempt = updates.lastFailedAttempt 
        ? updates.lastFailedAttempt.toISOString() 
        : null;
    }
  ...
  async logLoginAttempt(attempt: InsertLoginAttempt): Promise<LoginAttempt> {
    ...
    await db.collection("loginAttempts").doc(String(nextId)).set({
      ...log,
      createdAt: now.toISOString(),
    });
  ...
  async getLoginAttempts(userId?: number, limit: number = 50): Promise<LoginAttempt[]> {
    ...
    if (userId !== undefined) {
      ...
      snapshot = await db.collection("loginAttempts").get();
    }
  ...
  async createAnnouncement(data: InsertAnnouncement): Promise<Announcement> {
    ...
    await db.collection("announcements").doc(String(nextId)).set({
      ...announcement,
      createdAt: now.toISOString(),
      updatedAt: now.toISOString(),
    });
  ...
  async updateAnnouncement(id: number, data: Partial<InsertAnnouncement>): Promise<Announcement | null> {
    ...
    await db.collection("announcements").doc(String(id)).update({
      ...data,
  ...
  async getUserSettings(userId: number): Promise<UserSettings | null> {
    const db = await this.getDb();
    const snapshot = await db.collection("userSettings")
      .where("userId", "==", userId)
      .limit(1)
      .get();
  ...
  async updateUserSettings(userId: number, data: Partial<{ churchName: string }>): Promise<UserSettings> {
    ...
    if (existing) {
      ...
      await db.collection("userSettings").doc(String(existing.id)).update({
        ...data,
        updatedAt: now.toISOString(),
      });
    ...
    await db.collection("userSettings").doc(String(nextId)).set({
      ...settings,
      createdAt: now.toISOString(),
      updatedAt: now.toISOString(),
    });
  ...
  async createSupportTicket(data: InsertSupportTicket): Promise<SupportTicket> {
    ...
    await db.collection("supportTickets").doc(String(nextId)).set({
      ...ticket,
      createdAt: now.toISOString(),
      updatedAt: now.toISOString(),
    });
  ...
  async getSupportTicketById(id: number): Promise<SupportTicket | null> {
    const db = await this.getDb();
    const doc = await db.collection("supportTickets").doc(String(id)).get();
  ...
  async updateSupportTicket(id: number, data: UpdateSupportTicket): Promise<SupportTicket | null> {
    ...
    await db.collection("supportTickets").doc(String(id)).update(updateData);
  ...
  async deleteSupportTicket(id: number): Promise<boolean> {
    ...
    await db.collection("supportTickets").doc(String(id)).delete();
    return true;
  }
}

function getStorageImplementation(): IStorage {
