...
const MAX_SECURITY_ANSWER_ATTEMPTS = 3;
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/security-questions", async (req: Request, res: Response) => {
    try {
      ...
      if (user.isRecoveryBlocked) {
        return res.status(403).json({ 
          message: "Recuperação de senha bloqueada. Entre em contato com o suporte.",
      ...
      if (localQuestions) {
        ...
        if (questions.some(q => q && q.length > 0)) {
          return res.json({ 
            questions,
            source: "local",
            attemptsRemaining: MAX_SECURITY_ANSWER_ATTEMPTS - (user.securityAnswerAttempts || 0)
          });
        }
      }
  ...
  app.post("/api/auth/verify-security-answers", async (req: Request, res: Response) => {
    try {
      ...
      if (!isValid) {
        const attempts = await storage.incrementSecurityAnswerAttempts(user.id);
        const attemptsRemaining = MAX_SECURITY_ANSWER_ATTEMPTS - attempts;
        
        if (attempts >= MAX_SECURITY_ANSWER_ATTEMPTS) {
          await storage.blockRecovery(user.id);
          return res.status(403).json({ 
            message: "Você excedeu o número máximo de tentativas. Entre em contato com o suporte para desbloquear sua conta.",
            code: "RECOVERY_BLOCKED",
            attemptsRemaining: 0
          });
        }
        ...
        return res.status(401).json({ 
          message: `Respostas incorretas. Você tem ${attemptsRemaining} tentativa(s) restante(s).`,
          code: "INCORRECT_ANSWERS",
          attemptsRemaining
        });
      }

      await storage.resetSecurityAnswerAttempts(user.id);
      res.json({ valid: true });
    } catch (error) {
  ...
  app.post("/api/admin/create-admin", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const newAdmin = await storage.addUser(
        email,
        username,
        phone || "",
        password,
        "admin",
        {
          question1: securityQuestion1 || "Pergunta padrão",
          answer1: securityAnswer1 || "resposta",
          question2: securityQuestion2 || "Pergunta padrão",
          answer2: securityAnswer2 || "resposta",
          question3: securityQuestion3 || "Pergunta padrão",
          answer3: securityAnswer3 || "resposta",
        }
      );
