...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/login", async (req: Request, res: Response) => {
    try {
      ...
      const ipAttempts = await storage.getRecentFailedAttemptsByIp(
        ipAddress,
        RATE_LIMIT_WINDOW
      );
      ...
      if (user.failedAttempts >= MAX_FAILED_ATTEMPTS) {
        const lockoutEnd = user.lastFailedAttempt
          ? new Date(
              user.lastFailedAttempt.getTime() + LOCKOUT_MINUTES * 60000
            )
          : new Date();
  ...
  app.get("/api/admin/support", requireAdmin, async (_req: Request, res: Response) => {
    try {
